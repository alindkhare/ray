...
serve.add_service_dependencies("pipeline2","serve2","serve3")
serve.add_service_dependencies("pipeline2","serve1","serve3")

# Provision the PIPELINE (You can provision the pipeline only once)
serve.provision_pipeline("pipeline1")
serve.provision_pipeline("pipeline2")

# You can only create an endpoint for pipeline after provisioning the pipeline
serve.create_endpoint_pipeline("pipeline1", "/echo", blocking=True)
serve.create_endpoint_pipeline("pipeline2", "/reverse_echo", blocking=True)

time.sleep(2)

while True:
    resp = requests.get("http://127.0.0.1:8000/echo").json()
    print("from pipeline1")
    print(pformat_color_json(resp))

    print("...Sleeping for 2 seconds...")
    time.sleep(2)

    resp = requests.get("http://127.0.0.1:8000/reverse_echo").json()
    print("from pipeline2")
    print(pformat_color_json(resp))
